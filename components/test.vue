...
// Combine watch
watch([props.index, props.batchIndexes], (newVals, oldVals) => {
  const [nextIndex, nextBatchIndexes] = newVals
  const [prevIndex, prevBatchIndexes] = oldVals

  // Gérer les vidéos en petit
  handleListVideo(prevIndex, prevBatchIndexes[prevIndex].findIndex(v => v), false)
  handleListVideo(nextIndex, nextBatchIndexes[nextIndex].findIndex(v => v), true)

  // Réinitialiser la sélection de média et la vidéo en plein écran
  selectedMedia.value = { clientIndex: null, batchIndex: null, mediaIndex: null }
  fullscreenVideo.value = null
})
